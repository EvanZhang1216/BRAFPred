...
def main(seed, args):
    ...
    graph_dataset = Graph_Regression_Dataset('raw_data_QSAR.csv', smiles_field='smiles',
                                             label_field=label, normalize=True, seed=seed, batch_size=batch_size,
                                             a=len(label), max_len=500, addH=True)
    ...
    for epoch in range(80):
        ...
        if mse_new.item() < mse:
            mse = mse_new.item()
            stopping_monitor = 0
            np.save('{}/{}{}{}{}{}'.format(arch['path'], task, seed, arch['name'], trained_epoch, trained_epoch,
                                           pretraining_str),
                    [y_true, y_preds])
...
if __name__ == "__main__":
    ...
    RMSE_list.append(np.std(mse_list))
    print(mse_list)
    print(RMSE_list)
